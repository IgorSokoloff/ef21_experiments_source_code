...
class WorkerThreadMarina(threading.Thread):
  ...
  def run(self):
    ...
    if ncfg.i_use_vr_marina or ncfg.i_use_vr_biased_diana:
...
#            wcfg.cmd_output_ready.release()
        ...
        if wcfg.cmd == "bcast_g_c0":
            ...
            #================================================================================================================================
            delta_offset = 0
            g_batch_next_compressed = mult_param(alpha=1, x = g_batch_next)
            
            for t in range(len(g_batch_next_compressed)):
                offset = len(g_batch_next_compressed[t].flatten(0))
                delta_flatten[(delta_offset):(delta_offset + offset)] = g_batch_next_compressed[t].flatten(0)
                delta_offset += offset
            ...
            for t in range(len(g_batch_next_compressed)):
                offset = len(g_batch_next_compressed[t].flatten(0))
                g_batch_next_compressed[t].flatten(0)[:] = delta_flatten[(delta_offset):(delta_offset + offset)]
                delta_offset += offset
            #Now g_batch_next_compressed is compressed g_batch_next
            diff_1 = sub_params(g_batch_next_compressed, g_batch_next)
            diff_2 = sub_params(g_i, g_batch_next)
            
            if norm_of_param(diff_1) < norm_of_param(diff_2):
                g_i = g_batch_next_compressed
...
def main():
    ...
    nn_config.model_name = args.model #"resnet18"      # NN architecture
    ...
    nn_config.batch_size = args.batch_size#1024#128             # Technical batch size for training (due to GPU limitations)
    ...
    #=======================================================================================================
    K = args.k #100000
    gamma = args.factor*1e-3   #512*1e-3
    batch_size_for_worker = args.batch_size   #1024#256
    #=======================================================================================================
    ...
    #===================================================================================
    if nn_config.i_use_marina or nn_config.i_use_vr_marina or nn_config.i_use_vr_biased_diana:
        # Evaluate g0
        for i in range(kWorkers):
        ...
        for iteration in range(0, nn_config.KMax):
            ...
            ck = 0 #dont need this, it was from Bernoulli draw, effectlively it is always 0 for biased diana method
    ...
    if nn_config.i_use_vr_biased_diana: prefix4algo = "EF21_plus"
