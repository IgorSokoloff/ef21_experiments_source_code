...
def compute_full_funcs (A, x, b, la,n_workers):
    ...
    for i in range(n_workers):
        funcs_ar[i] = logreg_loss(x, A[i], b[i], la).copy()
    return funcs_ar
...
def biased_diana_top_k_gd(x_0, A, b, A_0, b_0, stepsize, eps,la,k, n_workers, Nsteps=100000):
    ...
    return np.array(it_ar), np.array(sq_norm_ar), x, np.array(f_ar), np.array(ef_upd_ar)

def save_data(its, f_grad_norms, sol, f_ar, ef_upd_ar, k_size, experiment_name, project_path, dataset):
...
for i in range(len(n_ar)):
    ...
    print(f"maxit: {nsteps}")
    print(f"{loss_func}")
    print(f"n: {n_ar[0]}")
    print("L_0: ", L_0)
    print("Lt: ", Lt)
    print("sqrt(beta/theta): ", np.sqrt(beta_ar[0]/theta_ar[0]))
    
    step_size_diana_tpc = np.minimum( (1/(L_0 + Lt*np.sqrt(2 * beta_ar/theta_ar))), theta_ar / (2*mu_0))*factor
    
    print(f"stepsize: {step_size_diana_tpc}")
    print(f"left {(1/(L_0 + Lt*np.sqrt(2 * beta_ar/theta_ar)))}; right {theta_ar / (2*mu_0)}")    
    experiment_name = "ef21-plus-full-grad_nw-{0}_{1}x".format(n_ar[0], factor)

    for k in range (len(k_ar)):
        
        results = biased_diana_top_k_gd(x_0, X, y, X_0, y_0, step_size_diana_tpc[k], eps,la, k_ar[k], n_ar[0], Nsteps=nsteps)
        ...
        f_ar = results[3]
        ef_upd_ar = results[4]
        save_data(its, norms, sol, f_ar, ef_upd_ar, k_ar[k], experiment_name, project_path,dataset)
